...
public class MineralCard extends Card{
    ...
    public String toString() {
        ...
        String cleavage = "Cleavage: "+getCleavageString() + " ("+ getCleavage()+")";
        String specificGravity = "Specific gravity: " + getSpecificGravity();
        String crustalAbundance = "Crustal abundance: "+getCrustalAbundanceString() + " ("+ getCrustalAbundance()+")";
        String economicValue = "Economic value: "+getEconomicValueString() + " (" + getEconomicValue()+")";
        String hardness = "Hardness: "+ getHardness();

        return flatLine +
                "\n|"+LeftPadding(name, flatLine.length()-2)+name+RightPadding(name, flatLine.length()-2)+"|\n" +
                dottedLine + "\n|" +
                LeftPadding(hardness, flatLine.length()-2)+hardness+RightPadding(hardness, flatLine.length()-2)+"|\n" +
                dottedLine + "\n|" +
                LeftPadding(specificGravity, flatLine.length()-2)+specificGravity+RightPadding(specificGravity, flatLine.length()-2)+"|\n" +
                dottedLine + "\n|" +
                LeftPadding(cleavage, flatLine.length()-2)+cleavage+RightPadding(cleavage, flatLine.length()-2)+"|\n" +
                dottedLine + "\n|" +
                LeftPadding(crustalAbundance, flatLine.length()-2)+crustalAbundance+RightPadding(crustalAbundance, flatLine.length()-2)+"|\n" +
                dottedLine + "\n|" + (char) 27 + "[4m" +
                LeftPadding(economicValue, flatLine.length()-2)+economicValue+RightPadding(economicValue, flatLine.length()-2)+(char)27+"[0m|\n";
    }
    ...
    public String Line() {
        String line = "";
        int length=0;
        int cleavage = ("Cleavage: "+getCleavageString() +" ("+ getCleavage()+")").length();
        int specificGravity = ("Specific gravity: " + getSpecificGravity()).length();
        int crustalAbundance = ("Crustal abundance: "+getCrustalAbundanceString() + " ("+ getCrustalAbundance()+")").length();
        int economicValue = ("Economic value: "+getEconomicValueString() + " (" + getEconomicValue()+")").length();
        int hardness = ("Hardness: "+ getHardness()).length();
        if (cleavage > length){
            length = cleavage;
        }
        if (specificGravity > length){
            length = specificGravity;
        }
        if (crustalAbundance > length){
            length = crustalAbundance;
        }
        if (economicValue > length){
            length = economicValue;
        }
        if (hardness > length){
            length = hardness;
        }
